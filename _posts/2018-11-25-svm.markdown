...
We can define a hyperplane $$ \mathcal{H} := \{x | f(x) = x^T \beta + \beta_0 = 0\} $$. Then, $$ f(x) $$ is the signed distance from a point $$ x $$ in feature space to the hyperplane $$ \mathcal{H} $$.
